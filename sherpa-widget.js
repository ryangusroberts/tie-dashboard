...
(function(){
  ...
  function fmt(s){ // light formatting: paragraphs + **bold** + bullet lines
  ...
  var sb=null, token=SUPABASE_ANON_KEY, greeted=false, busy=false;
  function initSb(cb){
  ...
  function build(){
    ...
    function ask(){
      var q=ta.value.trim(); if(!q||busy)return; busy=true;
      ta.value='';ta.style.height='auto';
      bubble('user',esc(q));
      var thinking=bubble('bot','<span class="shDots">Sherpa is thinking…</span>',true);
      fetch(FN_URL,{method:'POST',headers:{'Authorization':'Bearer '+token,'apikey':SUPABASE_ANON_KEY,'Content-Type':'application/json'},
        body:JSON.stringify({question:q,lesson:lessonId()})})
        ...
        .then(function(d){
          ...
          } else { bubble('bot','I hit a snag answering that. Try again in a moment, or reach out to your VM coach.',true); }
          busy=false;
        })
